...
def test_permission_request_template_describes_snippet_only_scope() -> None:
    ...
    assert "Public snippet search with links" in template
    assert "Metadata-only topic comparison views" in template
    assert "Metadata-only indexed-term browsing" in template
    assert "Metadata-only query explanation views" in template
    assert "per-term breakdowns" in template
    assert "Bounded topic dossiers, evidence packs, recap views, and claim checks" in template
    assert "metadata-only term/comparison caps" in template
    assert "term, coverage, comparison, bounded topic, recap, evidence-pack, and claim diagnostics" in template
